...
nonstaticfeatures = ['retweet_count','user_verified','user_friends_count','user_followers_count','user_favourites_count','geo_coordinates_available','num_hashtags','num_mentions','num_urls','num_media']
staticfeatures = Xnorm.columns.tolist()
...
for i in range (0,10):
    ...
    exp.set_name('sequentialbackwardk'+str(k)+'i'+str(i))
    ...
    sfs = SequentialFeatureSelector(svc, k_features=k, forward=True, scoring='f1', cv=cv,fixed_features=staticfeatures, verbose=2, n_jobs=-1)
